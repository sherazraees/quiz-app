...
const LoginScreen = ({navigation, user, getLogin, isLoading}) => {
  ...
  const handleSubmitPress = () => {
    // let dataToSend = {username: 'Stephen', password: 'welcome'};
    // getLogin(dataToSend);
    // return;
    setErrortext('');
    if (!userName) {
        ...
        <View>
          <KeyboardAvoidingView enabled>
            ...
            {user?.data?.status == 400 ? (
              <Text style={styles.errorTextStyle}>{user.data.message}</Text>
            ) : null}
            <TouchableOpacity
